...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('chapters').select('*').eq('course_id', id).eq('is_published', true).order('order_index', { ascending: true });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const { title, description, video_hls_url, duration_seconds } = await request.json();
  ...
  const { data: chapter, error } = await supabase.from('chapters').insert({ id: uuidv4(), course_id: id, title: title.trim(), description: description || null, video_hls_url, duration_seconds: duration_seconds || 0, order_index: (count || 0), is_published: true }).select().single();
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 });
