...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: chapter } = await supabase.from('chapters').select('course_id, courses(instructor_id, is_published)').eq('id', id).single();
  if (!chapter) return NextResponse.json({ success: false, error: 'Chapter not found.' }, { status: 404 });
  const course = chapter.courses as unknown as { instructor_id: string; is_published: boolean };
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (course.instructor_id !== session.user.id && session.user.role !== 'admin') return NextResponse.json({ success: false, error: 'Not your chapter.' }, { status: 403 });
  if (session.user.role !== 'admin' && course.is_published) {
    return NextResponse.json({ success: false, error: 'Published course videos can only be deleted by an admin.' }, { status: 403 });
  }
